...
def perform_login(database, username, password, hotel_id):
    # Buscando os usuários registrados no banco de dados
    users = Database.get_hotel_staff(database, hotel_id)
    ...
    while attempts > 1:
        ...
        for user in users:
            db_id, db_user, db_pass, db_role, db_hotel = user
            ...
            if db_user == username:
                ...
                if Database.compare_password(password, db_pass):
                    ...
                    return (db_id, db_user, db_pass, db_role, db_hotel)
        # Limpando o console
        os.system('cls')
